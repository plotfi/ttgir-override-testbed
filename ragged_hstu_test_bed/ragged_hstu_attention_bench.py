...
def profiler_or_nullcontext(enabled: bool, with_stack: bool):  # pyre-ignore [3]
    ...
    def _kineto_trace_handler(p: torch.profiler.profile) -> None:
        trace_url = "/tmp/libkineto_activities_{}.json".format(os.getpid(), )
        print(p.key_averages(group_by_input_shape=True).table(sort_by="self_cuda_time_total"))
...
def generate_hstu_timestamps(batch_size: int, seq_len: int) -> torch.Tensor:
    ...
    timestamp_day: torch.Tensor = (
        torch.pow(torch.rand(size=(batch_size, seq_len + 1), device="cuda"), exponent=-0.8).int() - 1).int()
    ...
    timestamps = torch.clamp(torch.abs(timestamp_day * 86400 + timestamp_second), max=4 * 10e6)
...
def main(
    ...
    num_targets = torch.randint(target_size, size=(batch_size, ), device=torch.device("cuda"))
    ...
    print(f"P50 latency is {ms:.5f} ms\n"
