...
def _ragged_hstu_attn_fwd_base_one_block(  # noqa: C901
    ...
    if ATTN_BIAS_TYPE == "fused":
        ...
        if USE_TIME_BIAS:
            ...
            ts_w = tl.load(TW + ts,  #eviction_policy='evict_last',
        ...
        if USE_POS_BIAS:
            ...
            if HAS_MULTIPLE_TARGETS:
                ...
                if INVALID_MASK_TYPE == "lower_triangular":
                    ...
                    pos_offs_n = tl.where(pos_offs_n > n_targets - 1, pos_offs_n, n_targets - 1)
            if HAS_MAX_POS_IND:
            ...
            pos_w = tl.load(PW + offs_pos_w,  #eviction_policy='evict_last',
                            #mask=mask_m[:, None] and mask_n[None, :],
                            )
    ...
    if HAS_MULTIPLE_TARGETS:
        if INVALID_MASK_TYPE == "lower_triangular":
            invalid_mask = invalid_mask and ((start_n + offs_n[None, :] < seq_len - n_targets) or
                                             (start_n + offs_n[None, :] == offs_m[:, None]))
        else:
            invalid_mask = invalid_mask and ((start_n + offs_n[None, :] >= n_targets) or
                                             (start_n + offs_n[None, :] == offs_m[:, None]))
    # pyre-ignore[61]
